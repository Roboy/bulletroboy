...
class BulletRoboy(Node):
    ...
    def publish_collision(self, collision):
        ...
        Args:
            collision (list): the contact points list output of pybullet getcontactPoints.
        ...
        self.get_logger().debug("Publishing collision.")
    ...
    def publish_collision_to_decomposer(self, collision):
        ...
        for pt in collision:
            if pt[9] > 0:
                ...
                #pt[5] == positionOnA in PyBullet docu
                contact_pt.position.x = pt[5][0]
                contact_pt.position.y = pt[5][1]
                contact_pt.position.z = pt[5][2] - 1.5 # TODO to parameter

                #pt[7] == contactNormalOnB in PyBullet docu
                contact_pt.contactnormal.x = pt[7][0]
                contact_pt.contactnormal.y = pt[7][1]
                contact_pt.contactnormal.z = pt[7][2] - 1.5

                # self.draw_force([contact_pt.position.x, contact_pt.position.y, contact_pt.position.z], [contact_pt.contactnormal.x, contact_pt.contactnormal.y, contact_pt.contactnormal.z], pt[9])
            ...
            msg = Collision(contact_points=contact_pts)

            self.get_logger().info("Publishing collision  %i" % msg.header.seq)
